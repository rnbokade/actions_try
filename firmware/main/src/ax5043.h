...
#define MIN_FXTAL                       10000000
#define MAX_FXTAL                       50000000
...
struct tx_params {
	...
	union {
		...
		struct fsk_params fsk;
		struct msk_params msk;
		struct afsk_params afsk;
	};
 ...
 */
struct ax5043_conf {
	uint32_t            f_xtal;           /**< The reference crystal frequency */
	...
	ax5043_millis       millis;           /**< Function pointer for getting
                                MCU time in ms */
	ax5043_tx_complete  tx_complete_clbk; /**< Pointer to a function that will
                                be called after the completion of a frame
                                transmission. Handy for RF switches or PAs */
	struct ax5043_conf_priv priv;         /**< Private information for the driver.
...
ax5043_tx_frame(struct ax5043_conf *conf, const uint8_t *in, uint32_t len,
                uint8_t preamble_len, uint8_t postamble_len,
                uint32_t timeout_ms);
...
int
ax5043_set_antsel(struct ax5043_conf *conf, uint8_t weak_pullup,
